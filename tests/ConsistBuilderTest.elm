    ...
    describe "Planning.Helpers"
        ...
        , describe "takeStockFromInventory"
            ...
            , test "takes one stock item of specified type from correct spawn point" <|
                \_ ->
                    ...
                    in
                    Expect.all
                        [ \_ -> taken |> Expect.equal (Just { id = 1, stockType = Locomotive })
                        , \_ ->
                            newInventories
                                |> List.head
                                |> Maybe.map .availableStock
                                |> Expect.equal (Just [ { id = 2, stockType = PassengerCar } ])
                        ]
                        ()
            , test "takes first matching item when multiple of same type exist" <|
                \_ ->
                    ...
                    in
                    taken
                        |> Expect.equal (Just { id = 5, stockType = Boxcar })
            , test "returns Nothing when stock type not available" <|
                \_ ->
                    let
                        inventories =
                            [ { spawnPointId = EastStation
                              , availableStock = [ { id = 1, stockType = Locomotive } ]
                              }
                            ]
                ...
                \_ ->
                    let
                        inventories =
                            ...
                            , { spawnPointId = WestStation
                              , availableStock = [ { id = 2, stockType = Boxcar } ]
                              }
                            ]
                    ...
                    newInventories
                        ...
                        |> List.head
                        |> Maybe.map .availableStock
                        |> Expect.equal (Just [ { id = 2, stockType = Boxcar } ])
        ...
        , describe "returnStockToInventory"
            [ test "returns empty list when inventories list is empty" <|
                \_ ->
                    returnStockToInventory EastStation [ { id = 1, stockType = Locomotive } ] []
                        |> Expect.equal []
                ...
                \_ ->
                    let
                        inventories =
                            ...
                            , { spawnPointId = WestStation
                              , availableStock = [ { id = 4, stockType = Boxcar } ]
                              }
                            ]
                    ...
                    newInventories
                        ...
                        |> List.head
                        |> Maybe.map .availableStock
                        |> Expect.equal (Just [ { id = 4, stockType = Boxcar } ])
            , test "handles returning empty list" <|
                \_ ->
                    ...
                    newInventories
                        |> List.head
                        |> Maybe.map .availableStock
                        |> Expect.equal (Just [ { id = 1, stockType = Locomotive } ])
